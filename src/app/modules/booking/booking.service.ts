...
import { getTransactionId } from "./booking.utils";
...
const createBooking = async (payload: Partial<IBooking>, userId: string) => {
   ...
   try {
      ...
      if (!user?.phone || !user?.address) {
         throw new AppError(status.BAD_REQUEST, "Please Update Your Profile To Book A Tour");
      }

      const tour = await Tour.findById(payload.tour).session(session).select("costFrom");

      if (!tour?.costFrom) {
         throw new AppError(status.BAD_REQUEST, "No Tour Cost Found!!!");
      }
